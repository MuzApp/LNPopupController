...
typedef NS_ENUM(NSUInteger, LNMarqueeType) {
...
IB_DESIGNABLE
@interface LNMarqueeLabel : UILabel
...
/** Defines the direction and method in which the `LNMarqueeLabel` instance scrolls.
 ...
 `LNMarqueeLabel` supports four types of scrolling: `LNMLLeftRight`, `LNMLRightLeft`, `LNMLContinuous`, and `LNMLContinuousReverse`.
 ...
 - `LNMLLeftRight` type is ONLY compatible with a label text alignment of `NSTextAlignmentLeft`.
 - `LNMLRightLeft` type is ONLY compatible with a label text alignment of `NSTextAlignmentRight`.
 - `LNMLContinuous` does not require a text alignment (it is effectively centered).
 - `LNMLContinuousReverse` does not require a text alignment (it is effectively centered).
 
 Defaults to `LNMLLeftRight`.
...
#else
@property (nonatomic, assign) LNMarqueeType marqueeType;
#endif
...
/** Defines the duration of the scrolling animation.
 ...
 scrolling cycle. Note that for `LNMLLeftRight` and `LNMLRightLeft`, a cycle consists of the animation away,
...
/** A buffer (offset) between the leading edge of the label text and the label frame.
 ...
 @note For `LNMLContinuous`-type labels, the smallest value of `leadingBuffer`, 'trailingBuffer`, and `fadeLength`
